...
impl Channel {
    ...
    fn select_sector(&self, dev_no: u8, sector: BlockSector) {
        ...
        outb(
            self.reg_device(),
            DEV_MBS | DEV_LBA | if dev_no == 1 { DEV_DEV } else { 0 } | ((sector >> 24) as u8),
        );
    }
...
impl Channel {
    ...
    fn wait_while_busy(&self) -> bool {
        for i in 0..3000 {
            if i == 700 {
                println!("{} busy, waiting...", String::from_utf8_lossy(&self.name));
            }

            if (inb(self.reg_alt_status()) & STA_BSY) == 0 {
                if i >= 700 {
                    println!("{} ok", String::from_utf8_lossy(&self.name));
                }
...
pub unsafe extern "C" fn ide_interrupt_handler() -> ! {
    fn inner() {
        for chan in CHANNELS.iter() {
            // TODO: compare interrupt vector number with chan.irq
            if true {
                let c = &mut chan.lock();
