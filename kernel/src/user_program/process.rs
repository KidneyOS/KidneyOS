...
fn copy_arguments_to_stack(thread: &mut ThreadControlBlock, argv: *const *const u8) -> Option<u32> {
    if argv.is_null() {
        return Some(0)
    }
    ...
    // + 1 for null
    let argument_list_length = slice.len() + 1;
    ...
    argument_list[slice.len()] = core::ptr::null();
...
pub fn execve(path: *const u8, argv: *const *const u8, _envp: *const *const u8) -> isize {
    ...
    let Some(ptr) = copy_arguments_to_stack(&mut thread, argv) else {
        return -E2BIG;
    };

    println!("Choosing: {ptr}");
